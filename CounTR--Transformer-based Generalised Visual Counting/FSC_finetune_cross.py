...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='./weights/FSC147.pth',
...
def main(args):
    ...
    try:
        ...
        # 第二阶段训练方式,用MAE自监督，所以，这里用的模型是model_mae_cross
        # define the model
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            for data_iter_step, (samples, gt_density, _, boxes, pos, m_flag, im_names) in enumerate(
                ...
                # 数据转移到GPU，并转换数据类型为torch.half(半精度浮点数), 减少内存占用加速计算
                ...
                with torch.cuda.amp.autocast():
                    # output's shape: [26, 384, 384]
                    output = model(samples, boxes, shot_num)

                # Compute loss function
                # 生成一个80%保留的掩码
                # 计算损失的平方误差再乘以mask并求平均
