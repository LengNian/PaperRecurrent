...
def load_image():
    ...
    for bbox in bboxes:
        ...
        # image's shape  batch, height, width
        bbox = image[:, y1:y2 + 1, x1:x2 + 1]
        bbox = transforms.Resize((64, 64))(bbox)
        boxes.append(bbox.numpy())

    boxes = np.array(boxes)
    boxes = torch.Tensor(boxes)  #  boxes's shape  3, 3, 64, 64
...
def run_one_image(samples, boxes, pos, model):
    ...
    torchvision.utils.save_image(fig, f'./output/Visualisation.png')
...
checkpoint = torch.load('./weights/FSC147.pth', map_location='cpu')
model_without_ddp.load_state_dict(checkpoint['model'], strict=False)
print("Resume checkpoint %s" % './weights/FSC147.pth')
