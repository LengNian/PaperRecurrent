    ...
    def __getitem__(self, idx):
        ...
        if file_name in self.images:
            ...
            if points.shape[0] > 0:     
                ...
                # 遍历边界框, 裁剪query中的patch, 并应用self.query_transform
                for box in boxes:
                    x1, y1 = box[0].astype(np.int32)
                    x2, y2 = box[2].astype(np.int32)
                    patch = img.crop((x1, y1, x2, y2))
                    patches.append(self.query_transform(patch))
                    # 计算每个patch的尺度嵌入, 保存到scale_embedding

                    # 这里对应论文中提到的scale embedding, 论文中提到的l_total其实就是这里的self.scale_number
                    # calculate scale
